class FileInfo
  ...
  def getoptivcodec(codec, hq = false)
    if codec.to_s == ''
      ...
      return('libx264')
    elsif codec.downcase.gsub("-", "") == "hvec" or codec.downcase.gsub("-", "") == "h265" or codec.downcase.gsub("-", "") == "h.265"
      return('libx265')
  ...
  def hdr_to_sdr(output)
    options = {
        custom: ['-c', 'copy',  '-max_muxing_queue_size', '40000', '-map', '0', '-vf', 'zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p', '-c:v', getoptivcodec(getvcodec)]
    }
