...
class MusicLibrary
  ...
  def self.organize_file(path, destination_root, folder_name: nil, no_prompt: 1)
    ...
    tags = merge_tags(read_tags(path), parse_from_names(File.basename(path, ".#{ext}"), folder_name))
    relative = build_relative_path(tags, ext, File.basename(path, ".#{ext}"))
    dest = File.join(destination_root, relative)
    if File.exist?(dest)
      app.speaker.speak_up("music organize: '#{File.basename(dest)}' already exists, skipping") if Env.debug?
      return dest
    end
