  ...
  def self.create_playlists(folder:, criteria: {}, move_untagged: '', remove_existing_playlists: 1, random: 0)
    ...
    files.each do |file_entry|
      ...
      unless valid_tags?(f_song)
        ...
        if $speaker.ask_if_needed(prompt, move_untagged.to_s != '' ? 1 : 0, 'y') == 'y'
          destination_folder = $speaker.ask_if_needed("Enter the full path of the folder to move the files into: ",
    ...
    CRITERIA_KEYS.each do |cr|
      ...
      if $speaker.ask_if_needed(prompt, default_choice, default_answer) == 'y'
